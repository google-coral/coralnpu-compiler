func.func @main(%arg0: tensor<?x?x?x?xi8>, %arg1: tensor<?x?x?x?xi8>) -> tensor<?x?x?x?x?xi32> {
  ...
  %c_in = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?xi8>
  ...
  %empty = tensor.empty(%n, %oh, %ow, %c_in, %m) : tensor<?x?x?x?x?xi32>
