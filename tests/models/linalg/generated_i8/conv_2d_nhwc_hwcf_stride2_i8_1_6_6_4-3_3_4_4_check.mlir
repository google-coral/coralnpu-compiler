module {
  func.func @"conv_2d_nhwc_hwcf_stride2_i8_1_6_6_4-3_3_4_4"() {
    %cst = arith.constant dense<[[[[24, 62, 100, -118], [-40, 30, 100, -86]], [[-104, 126, 100, 74], [88, 94, 100, 106]]]]> : tensor<1x2x2x4xi8>
    %cst_0 = arith.constant dense<""> : tensor<3x3x4x4xi8>
    %c0_i8 = arith.constant 0 : i8
    %cst_1 = arith.constant dense<""> : tensor<1x6x6x4xi8>
    %0 = util.optimization_barrier %cst_1 : tensor<1x6x6x4xi8>
    %1 = util.optimization_barrier %cst_0 : tensor<3x3x4x4xi8>
    %2 = tensor.empty() : tensor<1x2x2x4xi8>
    %3 = linalg.fill ins(%c0_i8 : i8) outs(%2 : tensor<1x2x2x4xi8>) -> tensor<1x2x2x4xi8>
    %4 = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%0, %1 : tensor<1x6x6x4xi8>, tensor<3x3x4x4xi8>) outs(%3 : tensor<1x2x2x4xi8>) -> tensor<1x2x2x4xi8>
    %5 = util.optimization_barrier %cst : tensor<1x2x2x4xi8>
    "check.expect_eq"(%4, %5) : (tensor<1x2x2x4xi8>, tensor<1x2x2x4xi8>) -> ()
    return
  }
}
