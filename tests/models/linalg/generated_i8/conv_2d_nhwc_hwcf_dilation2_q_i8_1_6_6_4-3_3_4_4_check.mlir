module {
  func.func @"conv_2d_nhwc_hwcf_dilation2_q_i8_1_6_6_4-3_3_4_4"() {
    %cst = arith.constant dense<[[[[77462, 79100, 80738, 82376], [84166, 85948, 87730, 89512]], [[111542, 111996, 112450, 112904], [118246, 118844, 119442, 120040]]]]> : tensor<1x2x2x4xi32>
    %cst_0 = arith.constant dense<""> : tensor<3x3x4x4xi8>
    %c0_i32 = arith.constant 0 : i32
    %c-5_i32 = arith.constant -5 : i32
    %c12_i32 = arith.constant 12 : i32
    %cst_1 = arith.constant dense<""> : tensor<1x6x6x4xi8>
    %0 = util.optimization_barrier %cst_1 : tensor<1x6x6x4xi8>
    %1 = util.optimization_barrier %cst_0 : tensor<3x3x4x4xi8>
    %2 = tensor.empty() : tensor<1x2x2x4xi32>
    %3 = linalg.fill ins(%c0_i32 : i32) outs(%2 : tensor<1x2x2x4xi32>) -> tensor<1x2x2x4xi32>
    %4 = linalg.conv_2d_nhwc_hwcf_q {dilations = dense<2> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>} ins(%0, %1, %c12_i32, %c-5_i32 : tensor<1x6x6x4xi8>, tensor<3x3x4x4xi8>, i32, i32) outs(%3 : tensor<1x2x2x4xi32>) -> tensor<1x2x2x4xi32>
    %5 = util.optimization_barrier %cst : tensor<1x2x2x4xi32>
    "check.expect_eq"(%4, %5) : (tensor<1x2x2x4xi32>, tensor<1x2x2x4xi32>) -> ()
    return
  }
}
