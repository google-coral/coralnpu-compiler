...
module {
  func.func @generic_reduction_2d_bf16_4_8() {
    %cst = arith.constant dense<[2.800000e+01, 9.200000e+01, 1.560000e+02, 2.200000e+02]> : tensor<4xbf16>
    ...
    %3 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%0 : tensor<4x8xbf16>) outs(%2 : tensor<4xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      %5 = arith.addf %in, %out : bf16
      linalg.yield %5 : bf16
    } -> tensor<4xbf16>
    %4 = util.optimization_barrier %cst : tensor<4xbf16>
    "check.expect_almost_eq"(%3, %4) {atol = 0.00999999977 : f32, rtol = 0.00999999977 : f32} : (tensor<4xbf16>, tensor<4xbf16>) -> ()
    return
  }
}
