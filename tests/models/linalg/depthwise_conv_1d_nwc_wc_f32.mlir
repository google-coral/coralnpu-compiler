func.func @main(%arg0: tensor<?x?x?xf32>, %arg1: tensor<?x?xf32>) -> tensor<?x?x?xf32> {
  ...
  %input_w = tensor.dim %arg0, %c1_idx : tensor<?x?x?xf32>
  %c = tensor.dim %arg0, %c2_idx : tensor<?x?x?xf32>

  %kernel_w = tensor.dim %arg1, %c0_idx : tensor<?x?xf32>
  ...
  %empty = tensor.empty(%n, %output_w, %c) : tensor<?x?x?xf32>
