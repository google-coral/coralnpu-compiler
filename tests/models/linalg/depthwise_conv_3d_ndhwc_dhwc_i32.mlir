func.func @main(%arg0: tensor<?x?x?x?x?xi32>, %arg1: tensor<?x?x?x?xi32>) -> tensor<?x?x?x?x?xi32> {
  ...
  %kernel_d = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi32>
  %kernel_h = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xi32>
  %kernel_w = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?xi32>
  ...
  %0 = linalg.depthwise_conv_3d_ndhwc_dhwc
       ...
       ins(%arg0, %arg1 : tensor<?x?x?x?x?xi32>, tensor<?x?x?x?xi32>)
