func.func @main(%arg0: tensor<?x?x?x?x?xi8>, %arg1: tensor<?x?x?x?x?xi8>) -> tensor<?x?x?x?x?xi32> {
  ...
  %g = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?x?xi8>
  %input_h = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?x?xi8>
  %input_w = tensor.dim %arg0, %c4_idx : tensor<?x?x?x?x?xi8>

  %c_out = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?x?xi8>
  %kernel_h = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?x?xi8>
  %kernel_w = tensor.dim %arg1, %c4_idx : tensor<?x?x?x?x?xi8>
  ...
  %empty = tensor.empty(%n, %g, %c_out, %oh, %ow) : tensor<?x?x?x?x?xi32>
