func.func @main(%arg0: tensor<?x?x?x?xi32>, %arg1: tensor<?x?x?x?xi32>) -> tensor<?x?x?x?xi32> {
  ...
  %c = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?xi32>
  %input_h = tensor.dim %arg0, %c2_idx : tensor<?x?x?x?xi32>
  %input_w = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?xi32>

  %f = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi32>
  %kernel_h = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?xi32>
  %kernel_w = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xi32>
  ...
  %empty = tensor.empty(%n, %f, %output_h, %output_w) : tensor<?x?x?x?xi32>
