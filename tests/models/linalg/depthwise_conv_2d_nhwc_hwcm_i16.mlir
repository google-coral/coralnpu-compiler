func.func @main(%arg0: tensor<?x?x?x?xi16>, %arg1: tensor<?x?x?x?xi16>) -> tensor<?x?x?x?x?xi16> {
  ...
  %kernel_h = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi16>
  %kernel_w = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xi16>
  %m = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xi16>
  ...
  %empty = tensor.empty(%n, %output_h, %output_w, %c, %m) : tensor<?x?x?x?x?xi16>
  ...
  %0 = linalg.depthwise_conv_2d_nhwc_hwcm
       ...
       ins(%arg0, %arg1 : tensor<?x?x?x?xi16>, tensor<?x?x?x?xi16>)
