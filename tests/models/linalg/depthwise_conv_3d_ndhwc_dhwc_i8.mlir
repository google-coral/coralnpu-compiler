func.func @main(%arg0: tensor<?x?x?x?x?xi8>, %arg1: tensor<?x?x?x?xi8>) -> tensor<?x?x?x?x?xi8> {
  ...
  %kernel_d = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi8>
  ...
  %0 = linalg.depthwise_conv_3d_ndhwc_dhwc
       ...
       ins(%arg0, %arg1 : tensor<?x?x?x?x?xi8>, tensor<?x?x?x?xi8>)
