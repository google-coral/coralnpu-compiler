func.func @main(%arg0: tensor<?x?x?x?xi8>, %arg1: tensor<?x?x?x?xi8>) -> tensor<?x?x?x?x?xi8> {
  ...
  %m = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xi8>
  ...
  %empty = tensor.empty(%n, %output_h, %output_w, %c, %m) : tensor<?x?x?x?x?xi8>
  ...
  %0 = linalg.depthwise_conv_2d_nhwc_hwcm
       ...
       ins(%arg0, %arg1 : tensor<?x?x?x?xi8>, tensor<?x?x?x?xi8>)
