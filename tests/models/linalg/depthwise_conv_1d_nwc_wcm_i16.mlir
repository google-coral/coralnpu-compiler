func.func @main(%arg0: tensor<?x?x?xi16>, %arg1: tensor<?x?x?xi16>) -> tensor<?x?x?x?xi16> {
  ...
  %kw = tensor.dim %arg1, %c0_idx : tensor<?x?x?xi16>
  %m = tensor.dim %arg1, %c2_idx : tensor<?x?x?xi16>

  %subw = arith.subi %w, %kw : index
  %c1 = arith.constant 1 : index
  %ow = arith.addi %subw, %c1 : index

  %empty = tensor.empty(%n, %ow, %c, %m) : tensor<?x?x?x?xi16>
