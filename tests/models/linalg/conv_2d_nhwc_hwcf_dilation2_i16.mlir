func.func @main(%arg0: tensor<?x?x?x?xi16>, %arg1: tensor<?x?x?x?xi16>) -> tensor<?x?x?x?xi16> {
  ...
  %kh = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi16>
  %kw = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xi16>
  %f = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xi16>
  ...
  %empty = tensor.empty(%n, %oh, %ow, %f) : tensor<?x?x?x?xi16>
  %fill = linalg.fill ins(%c0 : i16) outs(%empty : tensor<?x?x?x?xi16>) -> tensor<?x?x?x?xi16>
  %0 = linalg.conv_2d_nhwc_hwcf {dilations = dense<2> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>} ins(%arg0, %arg1 : tensor<?x?x?x?xi16>, tensor<?x?x?x?xi16>) outs(%fill : tensor<?x?x?x?xi16>) -> tensor<?x?x?x?xi16>
