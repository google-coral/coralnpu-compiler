func.func @main(%arg0: tensor<?x?x?x?xi32>, %arg1: tensor<?x?x?x?xi32>) -> tensor<?x?x?x?x?xi32> {
  ...
  %kernel_h = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi32>
  %kernel_w = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xi32>
  %m = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xi32>
  ...
  %empty = tensor.empty(%n, %output_h, %output_w, %c, %m) : tensor<?x?x?x?x?xi32>
