...
func.func @main(%arg0: tensor<?x?x?x?x?xf32>, %arg1: tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?x?xf32> {
  ...
  %kernel_d = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?x?xf32>
  %kernel_h = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?x?xf32>
  %kernel_w = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?x?xf32>
  %m = tensor.dim %arg1, %c4_idx : tensor<?x?x?x?x?xf32>
  ...
  %empty = tensor.empty(%n, %od, %oh, %ow, %c, %m) : tensor<?x?x?x?x?x?xf32>
  %fill = linalg.fill ins(%c0 : f32) outs(%empty : tensor<?x?x?x?x?x?xf32>) -> tensor<?x?x?x?x?x?xf32>
  ...
  %0 = linalg.depthwise_conv_3d_ndhwc_dhwcm
       ...
       outs(%fill : tensor<?x?x?x?x?x?xf32>) -> tensor<?x?x?x?x?x?xf32>
