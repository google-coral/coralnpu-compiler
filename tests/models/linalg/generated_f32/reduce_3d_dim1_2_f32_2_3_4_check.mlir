module {
  func.func @reduce_3d_dim1_2_f32_2_3_4() {
    %cst = arith.constant dense<[6.600000e+01, 2.100000e+02]> : tensor<2xf32>
    ...
    %2 = linalg.fill ins(%cst_0 : f32) outs(%1 : tensor<2xf32>) -> tensor<2xf32>
    %reduced = linalg.reduce ins(%0 : tensor<2x3x4xf32>) outs(%2 : tensor<2xf32>) dimensions = [1, 2] 
      (%in: f32, %init: f32) {
        %4 = arith.addf %in, %init : f32
        linalg.yield %4 : f32
      }
    %3 = util.optimization_barrier %cst : tensor<2xf32>
    "check.expect_almost_eq"(%reduced, %3) {rtol = 9.99999997E-7 : f32} : (tensor<2xf32>, tensor<2xf32>) -> ()
    return
  }
}
