...
module {
  func.func @generic_reduction_2d_f32_4_8() {
    %cst = arith.constant dense<[2.800000e+01, 9.200000e+01, 1.560000e+02, 2.200000e+02]> : tensor<4xf32>
    ...
    %3 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%0 : tensor<4x8xf32>) outs(%2 : tensor<4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %5 = arith.addf %in, %out : f32
      linalg.yield %5 : f32
    } -> tensor<4xf32>
    %4 = util.optimization_barrier %cst : tensor<4xf32>
    "check.expect_almost_eq"(%3, %4) {rtol = 9.99999997E-7 : f32} : (tensor<4xf32>, tensor<4xf32>) -> ()
    return
  }
}
