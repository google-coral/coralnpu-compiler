module {
  func.func @reduce_2d_max_f32_4_8() {
    %cst = arith.constant dense<[7.000000e+00, 1.500000e+01, 2.300000e+01, 3.100000e+01]> : tensor<4xf32>
    ...
    %reduced = linalg.reduce ins(%0 : tensor<4x8xf32>) outs(%2 : tensor<4xf32>) dimensions = [1] 
      (%in: f32, %init: f32) {
        %4 = arith.maximumf %in, %init : f32
        linalg.yield %4 : f32
      }
    %3 = util.optimization_barrier %cst : tensor<4xf32>
    "check.expect_almost_eq"(%reduced, %3) {rtol = 9.99999997E-7 : f32} : (tensor<4xf32>, tensor<4xf32>) -> ()
    return
  }
}
