func.func @main(%arg0: tensor<?x?x?x?x?xf32>, %arg1: tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> {
  ...
  %batch = tensor.dim %arg0, %c0_idx : tensor<?x?x?x?x?xf32>
  %m1 = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?x?xf32>
  %n1 = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?x?xf32>
  %m0 = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?x?xf32>
  %n0 = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?x?xf32>

  %empty = tensor.empty(%batch, %m1, %n1, %m0, %n0) : tensor<?x?x?x?x?xf32>
