func.func @main(%arg0: tensor<?x?x?xi8>, %arg1: tensor<?x?x?xi8>) -> tensor<?x?x?xi8> {
  ...
  %b = tensor.dim %arg0, %c0_idx : tensor<?x?x?xi8>
  %m = tensor.dim %arg0, %c1_idx : tensor<?x?x?xi8>
  %n = tensor.dim %arg1, %c2_idx : tensor<?x?x?xi8>
  %empty = tensor.empty(%b, %m, %n) : tensor<?x?x?xi8>
  %fill = linalg.fill ins(%c0 : i8) outs(%empty : tensor<?x?x?xi8>) -> tensor<?x?x?xi8>
  %0 = linalg.batch_matmul ins(%arg0, %arg1 : tensor<?x?x?xi8>, tensor<?x?x?xi8>) outs(%fill : tensor<?x?x?xi8>) -> tensor<?x?x?xi8>
