// RUN: %template_path
func.func @main(%arg0: tensor<?x?xi32>) -> tensor<?x?x8x16xi32> {
  ...
  %empty = tensor.empty(%outer0, %outer1) : tensor<?x?x8x16xi32>
  %0 = linalg.pack %arg0 padding_value(%pad_val : i32) inner_dims_pos = [0, 1] inner_tiles = [8, 16] into %empty : tensor<?x?xi32> -> tensor<?x?x8x16xi32>
  return %0 : tensor<?x?x8x16xi32>
}
