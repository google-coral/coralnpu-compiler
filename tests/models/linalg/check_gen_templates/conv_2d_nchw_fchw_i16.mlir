func.func @main(%arg0: tensor<?x?x?x?xi16>, %arg1: tensor<?x?x?x?xi16>) -> tensor<?x?x?x?xi16> {
  ...
  %kernel_h = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?xi16>
  %kernel_w = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xi16>
  ...
  %empty = tensor.empty(%n, %f, %output_h, %output_w) : tensor<?x?x?x?xi16>
