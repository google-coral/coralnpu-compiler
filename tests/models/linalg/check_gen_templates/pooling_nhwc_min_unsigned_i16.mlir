func.func @main(%arg0: tensor<?x?x?x?xi16>, %arg1: tensor<?x?xi16>) -> tensor<?x?x?x?xi16> {
  %max_ui16 = arith.constant -1 : i16
  ...
  %input_h = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?xi16>
  %input_w = tensor.dim %arg0, %c2_idx : tensor<?x?x?x?xi16>
  %c = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?xi16>

  %kernel_h = tensor.dim %arg1, %c0_idx : tensor<?x?xi16>
  %kernel_w = tensor.dim %arg1, %c1_idx : tensor<?x?xi16>
  ...
  %empty = tensor.empty(%n, %output_h, %output_w, %c) : tensor<?x?x?x?xi16>
  %fill = linalg.fill ins(%max_ui16 : i16) outs(%empty : tensor<?x?x?x?xi16>) -> tensor<?x?x?x?xi16>
  %0 = linalg.pooling_nhwc_min_unsigned
       {strides = dense<1> : vector<2xi64>, dilations = dense<1> : vector<2xi64>}
       ins(%arg0, %arg1 : tensor<?x?x?x?xi16>, tensor<?x?xi16>)
