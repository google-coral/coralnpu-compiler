func.func @main(%arg0: tensor<?x?x?x?xi8>, %arg1: tensor<?x?xi8>) -> tensor<?x?x?x?xi8> {
  %c0 = arith.constant -128 : i8 // Min i8 for max pooling init
  ...
  %c = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?xi8>
  ...
  %empty = tensor.empty(%n, %c, %output_h, %output_w) : tensor<?x?x?x?xi8>
