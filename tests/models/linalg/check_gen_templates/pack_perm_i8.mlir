...
func.func @main(%arg0: tensor<?x?xi8>) -> tensor<?x?x8x16xi8> {
  ...
  %empty = tensor.empty(%outer1, %outer0) : tensor<?x?x8x16xi8>
  %0 = linalg.pack %arg0 outer_dims_perm = [1, 0] inner_dims_pos = [0, 1] inner_tiles = [8, 16] into %empty : tensor<?x?xi8> -> tensor<?x?x8x16xi8>
