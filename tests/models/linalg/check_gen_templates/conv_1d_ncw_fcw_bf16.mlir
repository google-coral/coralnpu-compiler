func.func @main(%arg0: tensor<?x?x?xbf16>, %arg1: tensor<?x?x?xbf16>) -> tensor<?x?x?xbf16> {
  ...
  %c_in = tensor.dim %arg0, %c1_idx : tensor<?x?x?xbf16>
  %w = tensor.dim %arg0, %c2_idx : tensor<?x?x?xbf16>

  %c_out = tensor.dim %arg1, %c0_idx : tensor<?x?x?xbf16>
  %kw = tensor.dim %arg1, %c2_idx : tensor<?x?x?xbf16>

  %subw = arith.subi %w, %kw : index
  %c1 = arith.constant 1 : index
  %ow = arith.addi %subw, %c1 : index

  %empty = tensor.empty(%n, %c_out, %ow) : tensor<?x?x?xbf16>
