func.func @main(%arg0: tensor<?x?x?xbf16>, %arg1: tensor<?x?x?xbf16>) -> tensor<?x?x?xbf16> {
  ...
  %input_d = tensor.dim %arg0, %c0_idx : tensor<?x?x?xbf16>
  %input_h = tensor.dim %arg0, %c1_idx : tensor<?x?x?xbf16>
  %input_w = tensor.dim %arg0, %c2_idx : tensor<?x?x?xbf16>
  ...
  %empty = tensor.empty(%output_d, %output_h, %output_w) : tensor<?x?x?xbf16>
