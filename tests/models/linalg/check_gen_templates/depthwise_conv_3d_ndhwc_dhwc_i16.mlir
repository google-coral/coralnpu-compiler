func.func @main(%arg0: tensor<?x?x?x?x?xi16>, %arg1: tensor<?x?x?x?xi16>) -> tensor<?x?x?x?x?xi16> {
  ...
  %n = tensor.dim %arg0, %c0_idx : tensor<?x?x?x?x?xi16>
  %input_d = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?x?xi16>
  %input_h = tensor.dim %arg0, %c2_idx : tensor<?x?x?x?x?xi16>
  %input_w = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?x?xi16>
  %c = tensor.dim %arg0, %c4_idx : tensor<?x?x?x?x?xi16>

  %kernel_d = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xi16>
  %kernel_h = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xi16>
  %kernel_w = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?xi16>
  ...
  %empty = tensor.empty(%n, %output_d, %output_h, %output_w, %c) : tensor<?x?x?x?x?xi16>
  ...
  %0 = linalg.depthwise_conv_3d_ndhwc_dhwc
       ...
       ins(%arg0, %arg1 : tensor<?x?x?x?x?xi16>, tensor<?x?x?x?xi16>)
