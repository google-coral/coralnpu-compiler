func.func @main(%arg0: tensor<?x?x?x?xi32>, %arg1: tensor<?x?x?xi32>) -> tensor<?x?x?x?xi32> {
  ...
  %kh = tensor.dim %arg1, %c0_idx : tensor<?x?x?xi32>
  %kw = tensor.dim %arg1, %c1_idx : tensor<?x?x?xi32>
  ...
  %empty = tensor.empty(%n, %oh, %ow, %c) : tensor<?x?x?x?xi32>
  %fill = linalg.fill ins(%c0 : i32) outs(%empty : tensor<?x?x?x?xi32>) -> tensor<?x?x?x?xi32>
  %0 = linalg.depthwise_conv_2d_nhwc_hwc {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%arg0, %arg1 : tensor<?x?x?x?xi32>, tensor<?x?x?xi32>) outs(%fill : tensor<?x?x?x?xi32>) -> tensor<?x?x?x?xi32>
