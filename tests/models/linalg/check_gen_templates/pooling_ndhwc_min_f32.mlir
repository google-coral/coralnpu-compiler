func.func @main(%arg0: tensor<?x?x?x?x?xf32>, %arg1: tensor<?x?x?xf32>) -> tensor<?x?x?x?x?xf32> {
  ...
  %kernel_d = tensor.dim %arg1, %c0_idx : tensor<?x?x?xf32>
  %kernel_h = tensor.dim %arg1, %c1_idx : tensor<?x?x?xf32>
  %kernel_w = tensor.dim %arg1, %c2_idx : tensor<?x?x?xf32>
  ...
  %fill = linalg.fill ins(%cst : f32) outs(%empty : tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32>
  %0 = linalg.pooling_ndhwc_min
       {strides = dense<1> : vector<3xi64>, dilations = dense<1> : vector<3xi64>}
       ins(%arg0, %arg1 : tensor<?x?x?x?x?xf32>, tensor<?x?x?xf32>)
