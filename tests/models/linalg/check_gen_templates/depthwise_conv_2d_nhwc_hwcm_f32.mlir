func.func @main(%arg0: tensor<?x?x?x?xf32>, %arg1: tensor<?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> {
  ...
  %kernel_h = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xf32>
  %kernel_w = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xf32>
  %m = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xf32>
  ...
  %empty = tensor.empty(%n, %output_h, %output_w, %c, %m) : tensor<?x?x?x?x?xf32>
