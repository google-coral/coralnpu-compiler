func.func @main(%arg0: tensor<?x?x?x?xf32>, %arg1: tensor<?x?x?xf32>) -> tensor<?x?x?x?xf32> {
  ...
  %c = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?xf32>
  %input_h = tensor.dim %arg0, %c2_idx : tensor<?x?x?x?xf32>
  %input_w = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?xf32>
  ...
  %empty = tensor.empty(%n, %c, %oh, %ow) : tensor<?x?x?x?xf32>
