func.func @main(%arg0: tensor<?x?x?x?x?xi8>, %arg1: tensor<?x?x?x?x?xi8>) -> tensor<?x?x?x?x?xi32> {
  ...
  %kernel_d = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?x?xi8>
  %kernel_h = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?x?xi8>
  %kernel_w = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?x?xi8>
  %c_out = tensor.dim %arg1, %c4_idx : tensor<?x?x?x?x?xi8>
  ...
  %empty = tensor.empty(%n, %od, %oh, %ow, %c_out) : tensor<?x?x?x?x?xi32>
