func.func @main(%arg0: tensor<?x?x?xi32>, %arg1: tensor<?x?x?xi32>) -> tensor<?x?x?xi32> {
  ...
  %b = tensor.dim %arg0, %c0_idx : tensor<?x?x?xi32>
  %m = tensor.dim %arg0, %c1_idx : tensor<?x?x?xi32>
  %n = tensor.dim %arg1, %c2_idx : tensor<?x?x?xi32>
  %empty = tensor.empty(%b, %m, %n) : tensor<?x?x?xi32>
  %fill = linalg.fill ins(%c0 : i32) outs(%empty : tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
  %0 = linalg.batch_matmul ins(%arg0, %arg1 : tensor<?x?x?xi32>, tensor<?x?x?xi32>) outs(%fill : tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
