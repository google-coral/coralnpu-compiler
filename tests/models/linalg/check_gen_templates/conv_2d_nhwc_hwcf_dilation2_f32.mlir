func.func @main(%arg0: tensor<?x?x?x?xf32>, %arg1: tensor<?x?x?x?xf32>) -> tensor<?x?x?x?xf32> {
  ...
  %kh = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?xf32>
  %kw = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?xf32>
  %f = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?xf32>
  ...
  %empty = tensor.empty(%n, %oh, %ow, %f) : tensor<?x?x?x?xf32>
  %fill = linalg.fill ins(%c0 : f32) outs(%empty : tensor<?x?x?x?xf32>) -> tensor<?x?x?x?xf32>
  %0 = linalg.conv_2d_nhwc_hwcf {dilations = dense<2> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>} ins(%arg0, %arg1 : tensor<?x?x?x?xf32>, tensor<?x?x?x?xf32>) outs(%fill : tensor<?x?x?x?xf32>) -> tensor<?x?x?x?xf32>
