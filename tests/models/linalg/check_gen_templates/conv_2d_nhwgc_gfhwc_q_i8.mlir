func.func @main(%arg0: tensor<?x?x?x?x?xi8>, %arg1: tensor<?x?x?x?x?xi8>) -> tensor<?x?x?x?x?xi32> {
  ...
  %input_h = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?x?xi8>
  %input_w = tensor.dim %arg0, %c2_idx : tensor<?x?x?x?x?xi8>
  %g = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?x?xi8>

  %c_out = tensor.dim %arg1, %c1_idx : tensor<?x?x?x?x?xi8>
  %kernel_h = tensor.dim %arg1, %c2_idx : tensor<?x?x?x?x?xi8>
  %kernel_w = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?x?xi8>
  ...
  %empty = tensor.empty(%n, %oh, %ow, %g, %c_out) : tensor<?x?x?x?x?xi32>
  ...
  %0 = linalg.conv_2d_nhwgc_gfhwc_q
       ...
       ins(%arg0, %arg1, %izp, %kzp : tensor<?x?x?x?x?xi8>, tensor<?x?x?x?x?xi8>, i32, i32)
