...
func.func @main(%arg0: tensor<?x?x?x?x?xf32>, %arg1: tensor<?x?x?x?x?xf32>) -> tensor<?x?x?x?x?xf32> {
  ...
  %g = tensor.dim %arg0, %c1_idx : tensor<?x?x?x?x?xf32>
  %input_h = tensor.dim %arg0, %c3_idx : tensor<?x?x?x?x?xf32>
  %input_w = tensor.dim %arg0, %c4_idx : tensor<?x?x?x?x?xf32>

  %c_out = tensor.dim %arg1, %c0_idx : tensor<?x?x?x?x?xf32> // F is at index 0
  %kernel_h = tensor.dim %arg1, %c3_idx : tensor<?x?x?x?x?xf32>
  %kernel_w = tensor.dim %arg1, %c4_idx : tensor<?x?x?x?x?xf32>
  ...
  %empty = tensor.empty(%n, %g, %c_out, %oh, %ow) : tensor<?x?x?x?x?xf32>
