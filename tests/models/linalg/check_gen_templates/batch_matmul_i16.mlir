func.func @main(%arg0: tensor<?x?x?xi16>, %arg1: tensor<?x?x?xi16>) -> tensor<?x?x?xi16> {
  ...
  %b = tensor.dim %arg0, %c0_idx : tensor<?x?x?xi16>
  %m = tensor.dim %arg0, %c1_idx : tensor<?x?x?xi16>
  %n = tensor.dim %arg1, %c2_idx : tensor<?x?x?xi16>
  %empty = tensor.empty(%b, %m, %n) : tensor<?x?x?xi16>
  %fill = linalg.fill ins(%c0 : i16) outs(%empty : tensor<?x?x?xi16>) -> tensor<?x?x?xi16>
  %0 = linalg.batch_matmul ins(%arg0, %arg1 : tensor<?x?x?xi16>, tensor<?x?x?xi16>) outs(%fill : tensor<?x?x?xi16>) -> tensor<?x?x?xi16>
