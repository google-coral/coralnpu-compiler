module {
  func.func @"compare_eq_rank1_i8_256-256"() {
    %cst = arith.constant dense<true> : tensor<256xi1>
    %cst_0 = arith.constant dense<""> : tensor<256xi8>
    %0 = util.optimization_barrier %cst_0 : tensor<256xi8>
    %1 = util.optimization_barrier %cst_0 : tensor<256xi8>
    %2 = stablehlo.compare EQ, %0, %1, SIGNED : (tensor<256xi8>, tensor<256xi8>) -> tensor<256xi1>
    %3 = util.optimization_barrier %cst : tensor<256xi1>
    "check.expect_eq"(%2, %3) : (tensor<256xi1>, tensor<256xi1>) -> ()
    return
  }
}
