module {
  func.func @"minimum_rank1_i8_256-256"() {
    %cst = arith.constant dense<""> : tensor<256xi8>
    %0 = util.optimization_barrier %cst : tensor<256xi8>
    %1 = util.optimization_barrier %cst : tensor<256xi8>
    %2 = stablehlo.minimum %0, %1 : tensor<256xi8>
    %3 = util.optimization_barrier %cst : tensor<256xi8>
    "check.expect_eq"(%2, %3) : (tensor<256xi8>, tensor<256xi8>) -> ()
    return
  }
}
