func.func @main(%arg0: tensor<?x?x?xi8>) -> tensor<?x?x?x?xi8> {
  %dim0_0d_i8 = "stablehlo.get_dimension_size"(%arg0) {dimension = 0 : i64} : (tensor<?x?x?xi8>) -> tensor<i32>
  ...
  %dim1_0d_i8 = "stablehlo.get_dimension_size"(%arg0) {dimension = 1 : i64} : (tensor<?x?x?xi8>) -> tensor<i32>
  ...
  %dim2_0d_i8 = "stablehlo.get_dimension_size"(%arg0) {dimension = 2 : i64} : (tensor<?x?x?xi8>) -> tensor<i32>
  %dim2_0d_i64 = stablehlo.convert %dim2_0d_i8 : (tensor<i32>) -> tensor<i64>
  ...
  } : (tensor<?x?x?xi8>, tensor<4xi64>) -> tensor<?x?x?x?xi8>
