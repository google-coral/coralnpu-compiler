...
typedef struct iree_hal_coralnpu_event_t {
  iree_hal_resource_t resource;
  iree_allocator_t host_allocator;
} iree_hal_coralnpu_event_t;
...
iree_status_t iree_hal_coralnpu_event_create(
  ...
  *out_event = NULL;
  ...
  iree_hal_coralnpu_event_t *event = NULL;
  iree_status_t status =
      iree_allocator_malloc(host_allocator, sizeof(*event), (void **)&event);
  if (iree_status_is_ok(status)) {
    iree_hal_resource_initialize(&iree_hal_coralnpu_event_vtable,
                                 &event->resource);
...
static void iree_hal_coralnpu_event_destroy(iree_hal_event_t *base_event) {
  iree_hal_coralnpu_event_t *event = iree_hal_coralnpu_event_cast(base_event);
...
static const iree_hal_event_vtable_t iree_hal_coralnpu_event_vtable = {
    .destroy = iree_hal_coralnpu_event_destroy,
};
