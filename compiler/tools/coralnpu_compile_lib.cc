...
#include "compiler/tools/coralnpu_compile_lib.h"
...
int mlir::iree_compiler::runCoralNpuCompileMain(int argc, char **argv) {
  ...
  auto processBuffer = [&](iree_compiler_source_t *source) -> bool {
    ...
    switch (compileMode) {
      case CompileMode::std:
        ...
        /*** CoralNPU addition begin ***/
        if (compileTo == IREEVMPipelinePhase::End) {
          // 1. Stage 1: Compile to Stream phase
          ireeCompilerInvocationSetCompileToPhase(r.inv, "stream");
          if (!ireeCompilerInvocationPipeline(r.inv,
                                              IREE_COMPILER_PIPELINE_STD))
            return false;

          // 2. Intercept: Steal module and run custom passes
          MlirOperation mlirModuleOp =
              ireeCompilerInvocationExportStealModule(r.inv);
          ModuleOp moduleOp = cast<ModuleOp>(unwrap(mlirModuleOp));
          ...
          if (failed(pm.run(moduleOp))) {
            ...
            ireeCompilerInvocationImportStealModule(
                r.inv, mlirModuleOp);  // Restore for cleanup
            return false;
          }
          ireeCompilerInvocationImportStealModule(r.inv, mlirModuleOp);

          // 3. Stage 3: Resume compilation from Stream to End
          ireeCompilerInvocationSetCompileFromPhase(r.inv, "stream");
          ireeCompilerInvocationSetCompileToPhase(r.inv, "end");
          if (!ireeCompilerInvocationPipeline(r.inv,
                                              IREE_COMPILER_PIPELINE_STD))
            return false;
        } else {
          /*** CoralNPU addition end ***/

          // Fallback for explicit phase debugging (e.g. --compile-to=flow)
          if (!ireeCompilerInvocationPipeline(r.inv,
                                              IREE_COMPILER_PIPELINE_STD))
            return false;
        }
