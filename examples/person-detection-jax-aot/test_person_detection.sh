...
main() {
  ...
  bazel run --config=dev //examples/person-detection-jax-aot:export_person_detection -- \
    --output "${SCRIPT_DIR}/person_detection.mlir"
  ...
  bazel run --config=dev @iree_core//tools:iree-compile -- \
    ...
    "${SCRIPT_DIR}/person_detection.mlir" \
    -o "${SCRIPT_DIR}/person_detection.vmfb"

  echo
  echo "=== Phase 3: Running person detection ==="
  bazel run --config=dev //examples/person-detection-jax-aot:detect_person -- \
    --vmfb "${SCRIPT_DIR}/person_detection.vmfb" \
