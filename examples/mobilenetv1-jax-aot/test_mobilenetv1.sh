...
main() {
  ...
  bazel run --config=dev //examples/mobilenetv1-jax-aot:export_mobilenetv1 -- \
    --output "${SCRIPT_DIR}/mobilenet_v1.mlir"
  ...
  bazel run --config=dev @iree_core//tools:iree-compile -- \
    ...
    "${SCRIPT_DIR}/mobilenet_v1.mlir" \
    -o "${SCRIPT_DIR}/mobilenet_v1.vmfb"

  echo
  echo "=== Phase 3: Running MobileNetV1 classification ==="
  bazel run --config=dev //examples/mobilenetv1-jax-aot:classify_mobilenetv1 -- \
    --vmfb "${SCRIPT_DIR}/mobilenet_v1.vmfb" \
