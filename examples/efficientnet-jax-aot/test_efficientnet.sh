...
main() {
  ...
  bazel run --config=dev //examples/efficientnet-jax-aot:export_efficientnet -- \
    --output "${SCRIPT_DIR}/efficientnet_b0.mlir"
  ...
  bazel run --config=dev @iree_core//tools:iree-compile -- \
    ...
    "${SCRIPT_DIR}/efficientnet_b0.mlir" \
    -o "${SCRIPT_DIR}/efficientnet_b0.vmfb"

  echo
  echo "=== Phase 3: Running EfficientNetB0 classification ==="
  bazel run --config=dev //examples/efficientnet-jax-aot:classify_efficientnet -- \
    --vmfb "${SCRIPT_DIR}/efficientnet_b0.vmfb" \
