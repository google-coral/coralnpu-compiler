...
main() {
  echo "=== Phase 1: Downloading YOLOv8n (640x640) TFLite and converting to TOSA MLIR ==="
  bazel run --config=dev //examples/yolo-tflite-aot:export_yolo -- \
    --output "${SCRIPT_DIR}/yolo.mlir"
  ...
  echo
  echo "=== Phase 3: Running YOLO (640x640) detection ==="
  bazel run --config=dev //examples/yolo-tflite-aot:detect_yolo -- \
    --vmfb "${SCRIPT_DIR}/yolo.vmfb" \
