...
main() {
  ...
  bazel run --config=dev //examples/keyword-spotting-jax-aot:export_keyword_spotting -- \
    --output "${SCRIPT_DIR}/keyword_spotting.mlir"
  ...
  bazel run --config=dev @iree_core//tools:iree-compile -- \
    ...
    "${SCRIPT_DIR}/keyword_spotting.mlir" \
    -o "${SCRIPT_DIR}/keyword_spotting.vmfb"

  echo
  echo "=== Phase 3: Running keyword spotting ==="
  bazel run --config=dev //examples/keyword-spotting-jax-aot:spot_keyword -- \
    --vmfb "${SCRIPT_DIR}/keyword_spotting.vmfb" \
