...
main() {
  ...
  bazel run --config=dev //examples/resnet8-cifar10-jax-aot:export_resnet8 -- \
    --output "${SCRIPT_DIR}/resnet8.mlir"
  ...
  bazel run --config=dev @iree_core//tools:iree-compile -- \
    ...
    "${SCRIPT_DIR}/resnet8.mlir" \
    -o "${SCRIPT_DIR}/resnet8.vmfb"

  echo
  echo "=== Phase 3: Running CIFAR-10 classification ==="
  bazel run --config=dev //examples/resnet8-cifar10-jax-aot:classify_cifar10 -- \
    --vmfb "${SCRIPT_DIR}/resnet8.vmfb" \
