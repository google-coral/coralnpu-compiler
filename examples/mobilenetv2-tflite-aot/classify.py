...
def init_iree_func(vmfb_path):
  ...
  main_module = getattr(ctx.modules, "module", None)
  if main_module is None:
    for m in ctx.modules:
      if hasattr(m, "main"):
        main_module = m
        break
  if main_module is None or not hasattr(main_module, "main"):
    raise RuntimeError("Could not find module with 'main' function in context")
...
def main():
  ...
  try:
    if output_np.shape[-1] == 1001:
      # Exclude background class at index 0 for 1000-class ImageNet decoding
      probs = output_np[:, 1:]
    else:
      probs = output_np

    results = keras.applications.mobilenet_v2.decode_predictions(probs,
                                                                 top=5)[0]
    print("\nPredictions:")
